...
{{
  "extracted_data": {{
    ...
    "document_subtype": "loan|rental|service|purchase|other",
    "parties": {{"party1": "...", "party2": "..."}},
    "total_amount": 100000 или null,
    "currency": "RUB",
    "dates": {{
      "signature": "2024-01-01" или null,
      "start_date": "2024-01-01" или null,
      "end_date": "2024-12-31" или null,
      "payment_due": "2024-01-15" или null
    }},
    "financial_terms": {{
      "interest_rate": "0.8% в день" или null,
      "interest_rate_annual": "292%" или null,
      "penalty_amount": "10% от суммы" или null,
      "payment_schedule": "ежемесячно" или null,
      "early_repayment": "разрешено/запрещено" или null
    }},
    "rental_terms": {{
      "monthly_rent": 50000 или null,
      "deposit": 50000 или null,
      "utilities": "арендатор платит" или null
    }},
...
• document_subtype: для договоров укажи тип (loan=заём, rental=аренда, service=услуги, purchase=купля-продажа)
• financial_terms: заполняй только если применимо (для займов, кредитов)
• rental_terms: заполняй только для договоров аренды
• requisites: извлеки ИНН/КПП если есть в тексте
• confidence_score: 0.0-1.0 (1.0 = полностью уверен)
• Верни ТОЛЬКО JSON, без пояснений и markdown

🎯 ОСОБОЕ ВНИМАНИЕ:
• Для займов/кредитов: обязательно извлеки процентную ставку (в день и годовых)
• Для аренды: укажи сумму аренды в месяц и залог
• Для счетов: укажи срок оплаты и список товаров/услуг
• Ищи скрытые риски: односторонние штрафы, автоматическая пролонгация, запрет на досрочное погашение
"""
