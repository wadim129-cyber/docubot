...
class DocumentAgent:
    ...
    def analyze_document(self, text: str) -> AnalysisResult:
        ...
        combined_prompt = f"""
Ты — эксперт по анализу юридических документов. Определи тип документа и извлеки соответствующие данные. Верни ТОЛЬКО валидный JSON.
...
{{
  "extracted_data": {{
    ...
    "financial_terms": {{
      "interest_rate": "0.8% в день",
      "loan_term": "30 дней",
      "penalties": "10% от суммы",
      "payment_schedule": "ежемесячно"
    }},
    "rental_terms": {{
      "monthly_rent": 50000,
      "deposit": 50000,
      "utilities": "арендатор платит"
    }},
...
⚠️ ПРАВИЛА:
• parties — СПИСОК: ["ООО ВЭББАНКИР", "Иванов Иван Иванович"]
• document_subtype: 
  - loan = заём/кредит (заполни financial_terms)
  - rental = аренда (заполни rental_terms)
  - service = услуги (укажи items)
  - purchase = купля-продажа (укажи items)
  - invoice = счёт (укажи items и payment_due)
  - act = акт (укажи items)
• Заполняй только те поля которые ПРИМЕНИМЫ к этому типу документа
• Если данных нет — ставь null
• Верни ТОЛЬКО JSON, без markdown
• confidence_score: 0.0-1.0

🎯 ДЛЯ КАЖДОГО ТИПА:
• Заём/кредит: interest_rate, loan_term, penalties
• Аренда: monthly_rent, deposit, utilities
• Счёт: items, payment_due, total_amount
• Акт: items, dates
"""
